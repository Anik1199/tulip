...
#include <time.h>   //CONN-EC-WIFI-DMS06298620-01+
...
void wpa_ctrl_cleanup(void)
{
	...
	while (readdir_r(dir, &entry, &result) == 0 && result != NULL) {
		...
		if (os_strlcpy(namep, entry.d_name, maxcopy) < maxcopy) {
			if (stat(pathname, &socket_file) == 0) {
				time(&now);
				diff = difftime(now, socket_file.st_atime);
				if (diff >= 5 * 60) {  //time of last access exceeds 5 min.
					unlink(pathname);
				}
			}
		}
