					...
					struct wpa_ssid *new_ssid)
{
	...
	for (ssid = wpa_s->conf->ssid, next = ssid ? ssid->next : NULL; ssid;
		...
		if (ssid->wps_run && ssid->wps_run == new_ssid->wps_run &&
			 ...
			 */
			//CONN-WIFI-CD-CONF_READ_ERR-01
			// Avoid new network to create new profile if proto and pairwise_cipher
			// are difference, because framework cannot normal handle this condition.
