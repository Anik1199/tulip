				       ...
				       union wpa_event_data *data)
{
	...
	if (wpa_s->pending_eapol_rx) {
		...
		if (age.sec == 0 && age.usec < 900000 && //CONN-FY-Extend_EAPOL_RX_frames_timeout
						 ...
						 int locally_generated)
{
	...
	if (wpa_s->current_ssid && wpa_s->current_ssid->no_auto_connect)
		wpa_s->current_ssid->disabled_for_auto_connect = 1;
	bssid = wpa_s->bssid;
	if (is_zero_ether_addr(bssid))
		bssid = wpa_s->pending_bssid;
