			      ...
			      enum wpa_states state)
{
	...
	wpa_dbg(wpa_s, MSG_INFO, "State: %s -> %s",    //CONN-FY-WIFI-PortingSupplicantDebugTool
...
static void wpa_supplicant_scan_delay_timeout(void *eloop_ctx)
{
	struct wpa_supplicant *wpa_s = eloop_ctx;

	wpa_s->scan_delay_in_progress = 0;
	if (wpa_s->scan_pending) {
		wpa_supplicant_req_scan(wpa_s, 0, 0);
		wpa_s->scan_pending = 0;
	}
}


/**
 * wpa_supplicant_set_scan_delay - Set scan delay
 * @wpa_s: wpa_supplicant structure for a network interface
 * @scan_delay: scan delay in milliseconds
 * Returns: 0 if succeed or -1 if scan_delay has an invalid value or scan delay
 * is in progress.
 */
int wpa_supplicant_set_scan_delay(struct wpa_supplicant *wpa_s, int scan_delay)
{
	if (scan_delay <= 0 ||
	    eloop_is_timeout_registered(wpa_supplicant_scan_delay_timeout,
					wpa_s, NULL))
		return -1;

	eloop_register_timeout(scan_delay / 1000, scan_delay % 1000 * 1000,
			       wpa_supplicant_scan_delay_timeout,
			       wpa_s, NULL);
	wpa_s->scan_delay_in_progress = 1;
	return 0;
}
						 ...
						 struct wpa_interface *iface)
{
	...
	wpa_supplicant_set_scan_delay(
		wpa_s, wpa_s->global->params.first_scan_delay_msec);
...
struct wpa_global * wpa_supplicant_init(struct wpa_params *params)
{
	...
	if (params->first_scan_delay_msec)
		global->params.first_scan_delay_msec =
			params->first_scan_delay_msec;
