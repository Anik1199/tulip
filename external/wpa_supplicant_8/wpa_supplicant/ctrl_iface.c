	...
	struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	if (os_strcmp(cmd, "all") == 0) {
		...
		while (ssid) {
			...
			ssid = ssid->next;  //CONN-FY-FixNullPoint_Crash
		}
		return 0;
	}
...
static int wpa_supplicant_ctrl_iface_scan_delay(
	struct wpa_supplicant *wpa_s, char *cmd)
{
	int scan_delay = atoi(cmd);
	return wpa_supplicant_set_scan_delay(wpa_s, scan_delay);
}
					 ...
					 char *buf, size_t *resp_len)
{
	...
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
		...
		if (wpa_supplicant_ctrl_iface_scan_interval(wpa_s, buf + 14))
			reply_len = -1;
	} else if (os_strncmp(buf, "SCAN_DELAY ", 11) == 0) {
		if (wpa_supplicant_ctrl_iface_scan_delay(wpa_s, buf + 11))
			reply_len = -1;
